...
| # | Task | Priority | Notes |
|---|------|----------|-------|
| T-015 | Verify BUG-0013 / 0014 / 0015 / 0016 / 0019 / 0020 fixes on Colab | High | **Startup confirmed 2026-08-25** (WebUI reached `Running on public URL` + `Connected`, ControlNet v1.1.455 loaded, no script UI errors). **Model load still failing as of 2026-08-26** (BUG-0020, open_clip 3.x layout). Still to confirm: model load, image generation, that the restored sed + transformers pin remove the ~12 GB of per-session downloads, and the BUG-0018 mount guard. |
